...
from water_generator.single_node_infra_sim import FloodSimEnv
...
def update(frame, polygons, points, ax, xmin, xmax, ymin, ymax, flood_frames, post_flood_frames, envs, activated):
    '''
    ...
    if frame < flood_frames:
        ...
        flood_x = xmax - (xmax - xmin) * smooth_progress
    else:
        flood_x = xmin
    ...
    for precinct_no, poly in polygons.items():
        if poly.geom_type == 'Polygon':
            ...
            if (not activated[precinct_no]) and (point_x >= flood_x):
...
if __name__ == "__main__":
    ...
    districts_file = gpd.read_file('Precinct_Boundaries.geojson')
    areas = extract_polygons(districts_file)

    points_file = gpd.read_file('random_points.geojson')
    ...
    fargs=(polygons, points, ax, xmin, xmax, ymin, ymax, flood_frames, post_flood_frames, envs, activated),
